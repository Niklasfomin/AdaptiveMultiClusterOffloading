...
import subprocess
import time
from typing import Any,List, AsyncGenerator, Generator, Optional, Self
import uuid
...
from snakemake_executor_plugin_kubernetes.helpers import get_uuid, read_and_base64_encode_file, convert_to_cpus, \
    convert_to_bytes
...
class ExecutorSettings(ExecutorSettingsBase):
    ...
    shared_workdir: Optional[str] = field(
        default=None,
        metadata={
            "help": "If set, run each job with working_dir inside this shared (NFS) "
            "directory and do not mount the default emptyDir /workdir. "
            "Use together with --<executor>-persistent-volumes "
    ...
    gcp_app_creds_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "Specify path to GCP application credentials file to be mounted into containers. Use jointly with"
                    "GCS storage provider plugin. Avoids the need of setting up workload identity federation."
        },
    )
    log_resource_utilization: bool = field(
        default=False,
        metadata={
            "help": "Log current cluster resource utilization when jobs complete. Default: False.",
            "is_flag": True,
        },
    )
...
common_settings = CommonSettings(
    ...
    # In NFS / shared-mount mode, the offloader overrides these via the
    # outer executor's common_settings; keeping them False here as well
    # avoids Snakemake requiring --default-storage-provider ftp when the
    # outer dispatcher is the offloader plugin.
...
class Executor(RemoteExecutor):
    def __post_init__(self):
        ...
        self.context = self.workflow.executor_settings.context if hasattr(self.workflow.executor_settings, "context") else None
        try:
            kubernetes.config.load_kube_config(context=self.context)
        except kubernetes.config.config_exception.ConfigException as e:
            self.logger.warning(f"Failed to load kube_config: {e} \n Trying in-cluster config.")
            kubernetes.config.load_incluster_config()

        self.gcp_app_creds_file = self.workflow.executor_settings.gcp_app_creds_file
        self.log_resource_utilization = self.workflow.executor_settings.log_resource_utilization
        ...
        if self.log_resource_utilization:
            self.logger.info(f"\nInitially unreserved resources:")
    ...
    def run_job(self, job: JobExecutorInterface):
        ...
        # This adapts the pip install command for the ftp storage plugin to work with test.pypi.org
        # The adapted version is used as the official version does not support uploading directories
        if "snakemake-storage-plugin-ftp-leshl" in exec_job:
            exec_job = exec_job.replace(
                "pip install --target '.snakemake/pip-deployments' snakemake-storage-plugin-ftp-leshl",
                "pip install --index-url https://test.pypi.org/simple --extra-index-url https://pypi.org/simple --target '.snakemake/pip-deployments' snakemake-storage-plugin-ftp-leshl"
            )
        ...
        if self.shared_workdir:
            # NFS / shared-filesystem mode: use the shared mount as workdir.
            # The user is expected to mount the shared PVC (see
            # --<executor>-persistent-volumes) at the same absolute path.
            container.working_dir = self.shared_workdir
        ...
        if self.gcp_app_creds_file:
            container.volume_mounts.append(
                kubernetes.client.V1VolumeMount(
                    name="gcp-credentials-volume",
                    mount_path="/var/secrets/gloud",
                    read_only=True,
                )
            )
        ...
        if (
            ...
            # this should work, but it doesn't currently because of
        ...
        if self.gcp_app_creds_file:
            volume = kubernetes.client.V1Volume(
                name="gcp-credentials-volume",
                secret=kubernetes.client.V1SecretVolumeSource(secret_name="gcp-credentials")
            )
            pod_spec.volumes.append(volume)
            ...
            async with self.status_rate_limiter:
                try:
                    res = self._kubernetes_retry(
                        lambda job=j: self.batchapi.read_namespaced_job_status(
                            job.external_jobid, self.namespace
                        )
                    )
                except kubernetes.client.rest.ApiException as e:
                ...
                pods = self._kubernetes_retry(
                    lambda job=j: self.kubeapi.list_namespaced_pod(
                        namespace=self.namespace,
                        label_selector=f"job-name={job.external_jobid}",
                    )
                )
                ...
                if pods.items:
                    pod = pods.items[0]
                    if pod.status.phase == "Running" and j.external_jobid in self.pending_jobs:
                        self.logger.info(datetime.now().strftime("[%a %b %e %H:%M:%S %Y]"))
                ...
                if (res.status.failed and res.status.failed > 0) or (
                    ...
                    self._kubernetes_retry(
                        lambda job=j: self.safe_delete_job(
                            job.external_jobid, ignore_not_found=True
                        )
                    )
                    ...
                    if self.log_resource_utilization:
                        self.logger.info("\nUnreserved resources after job completion:")
                        self.report_k8s_resource_utilization()
                else:
                    # still active
    ...
    def register_secret(self):
        ...
        # GCP application credentials file
        gcp_dict = {"GOOGLE_APPLICATION_CREDENTIALS": "/var/secrets/gloud/application_credentials.json"}
        env_vars = (self.envvars() | gcp_dict) if self.gcp_app_creds_file else self.envvars()
        ...
        # GCP application credentials file
        if self.gcp_app_creds_file:
            # ensure secret does not exist yet
            try:
                self.kubeapi.delete_namespaced_secret(name="gcp-credentials", namespace=self.namespace)
            except kubernetes.client.ApiException:
                pass

            encoded_credentials = read_and_base64_encode_file(self.gcp_app_creds_file)
            secret = kubernetes.client.V1Secret(
                metadata=kubernetes.client.V1ObjectMeta(name="gcp-credentials"),
                data={"application_credentials.json": encoded_credentials}
            )
            self.kubeapi.create_namespaced_secret(self.namespace, body=secret)
    ...
    def safe_delete_job(self, jobid, ignore_not_found=True, delete_completed=True):
        import kubernetes.client
        body = kubernetes.client.V1DeleteOptions()
    ...
    def get_k8s_resource_utilization(self):
        # Get all nodes and their allocatable resources
        ...
        for node in nodes.items:
            ...
            node_resources[node.metadata.name] = {
                "cpu_allocatable": convert_to_cpus(allocatable.get("cpu", "0")),
                "memory_allocatable": convert_to_bytes(allocatable.get("memory", "0")),
                "disk_allocatable": convert_to_bytes(allocatable.get("ephemeral-storage", "0")),
                "cpu_requested": 0,
                "memory_requested": 0,
                "disk_requested": 0,
                "cpu_unreserved": 0,
        ...
        # Get all running pods and sum up their resource requests per node
        pods = self.kubeapi.list_pod_for_all_namespaces()
        for pod in pods.items:
            if pod.spec.node_name:
                for container in pod.spec.containers:
                    if container.resources and container.resources.requests:
                        ...
                        disk_request = container.resources.requests.get(
                            "ephemeral-storage",
                            "0",
                        )

                        # Convert and sum up resources
                        node_resources[pod.spec.node_name]["cpu_requested"] += convert_to_cpus(
                            cpu_request
                        )
                        node_resources[pod.spec.node_name]["memory_requested"] += convert_to_bytes(
                            memory_request
                        )
                        node_resources[pod.spec.node_name]["disk_requested"] += convert_to_bytes(
                            disk_request
                        )

        # Compute and display free resources
        for node, resources in node_resources.items():
            resources["cpu_unreserved"] = resources["cpu_allocatable"] - resources["cpu_requested"]
            resources["memory_unreserved"] = (
                resources["memory_allocatable"] - resources["memory_requested"]
            )
            resources["disk_unreserved"] = resources["disk_allocatable"] - resources["disk_requested"]

        return node_resources


    def report_k8s_resource_utilization(self):
        node_resources = self.get_k8s_resource_utilization()
        for node, resources in node_resources.items():
            self.logger.info(f"  Node: {node}")
            self.logger.info(f"  CPU: {resources['cpu_unreserved']:.2f} cores")
            self.logger.info(f"  Memory: {(resources['memory_unreserved'] / 10 ** 6):.2f} MB")
            self.logger.info(f"  Disk Space: {(resources['disk_unreserved']/ 10 ** 6):.2f} MB")
